...
from . import edit_delete, jakootelbot, logging
...
autopic_path = os.path.join(os.getcwd(), "jakootelbot", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "jakootelbot", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "jakootelbot", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await jakootelbot.upload_file(autophoto_path)
        try:
            if i > 0:
                await jakootelbot(
                    functions.photos.DeletePhotosRequest(
                        await jakootelbot.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await jakootelbot(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await jakootelbot(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await jakootelbot(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@jakootelbot.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
@jakootelbot.ar_cmd(pattern="اسم وقتي$")
async def _(event):
...
@jakootelbot.ar_cmd(pattern="بايو وقتي$")
async def _(event):
...
@jakootelbot.ar_cmd(pattern="انهاء ([\s\S]*)")
async def _(event):
...
jakootelbot.loop.create_task(digitalpicloop())
jakootelbot.loop.create_task(autoname_loop())
jakootelbot.loop.create_task(autobio_loop())
