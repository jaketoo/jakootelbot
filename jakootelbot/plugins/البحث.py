...
from . import jakootelbot, song_download
...
@jakootelbot.ar_cmd(pattern="بحث(320)?(?:\s|$)([\s\S]*)")
...
@jakootelbot.ar_cmd(pattern="فيديو(?:\s|$)([\s\S]*)")
...
@jakootelbot.ar_cmd(pattern="(ا(ل)?ا(س)?م)(?:\s|$)([\s\S]*)")
    ...
    if flag == "s":
        ...
        async with event.client.conversation(chat) as conv:
            try:
                ...
                await jakootelbot(unblock("DeezerMusicBot"))
                purgeflag = await conv.send_message("/start")
