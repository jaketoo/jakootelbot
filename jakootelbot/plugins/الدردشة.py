...
async def ban_user(chat_id, i, rights):
    try:
        await jakootelbot(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@jakootelbot.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@jakootelbot.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@jakootelbot.on(events.NewMessage(outgoing=False, pattern="/roz"))
async def _(event):
...
@jakootelbot.on(events.NewMessage(outgoing=False, pattern="^تحية"))
async def _(event):
    user = await event.get_sender()
...
@jakootelbot.ar_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    ...
    try:
        await jakootelbot(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in jakootelbot.iter_participants(chat_id):
        ...
        await jakootelbot.send_message(chat_id, usrtxt)
...
@jakootelbot.ar_cmd(pattern="الغاء التفليش", groups_only=True)
