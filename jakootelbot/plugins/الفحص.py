...
from jakootelbot import StartTime, jakootelbot, jmthonversion
...
@jakootelbot.ar_cmd(pattern="فحص$")
...
@jakootelbot.ar_cmd(pattern="السورس$")
async def repo(event):
    RR7PP = Config.TG_BOT_USERNAME
    if event.reply_to_msg_id:
        await event.get_reply_message()
    response = await jakootelbot.inline_query(RR7PP, "السورس")
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("السورس") and event.query.user_id == jakootelbot.uid:
            buttons = [
                [
