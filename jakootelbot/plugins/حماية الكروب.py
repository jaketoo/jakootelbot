...
@jakootelbot.ar_cmd(pattern="قفل ?(.*)", groups_only=True, require_admin=True)
async def _(event):
...
@jakootelbot.ar_cmd(pattern="فتح ?(.*)", groups_only=True, require_admin=True)
async def _(event):
...
@jakootelbot.ar_cmd(pattern="الاعدادات$", groups_only=True)
async def _(event):
    res = ""
...
@jakootelbot.ar_cmd(incoming=True, forword=None)
...
@jakootelbot.on(events.ChatAction())
async def _(event):
...
@jakootelbot.ar_cmd(pattern=f"البوتات ?(.*)")
