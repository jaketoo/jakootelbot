...
async def delpack(jmthonevent, conv, args, packname):
    try:
        ...
        await jakootelbot(unblock("stickers"))
        await conv.send_message("/delpack")
...
@jakootelbot.ar_cmd(pattern="ملصق(?:\s|$)([\s\S]*)")
async def kang(args):
    photo = None
    emojibypass = False
    is_anim = False
...
@jakootelbot.ar_cmd(pattern="حزمة(?:\s|$)([\s\S]*)")
...
@jakootelbot.ar_cmd(pattern="فاس$")
...
@jakootelbot.ar_cmd(pattern="تحويل بملصق(?:\s|$)([\s\S]*)")
...
@jakootelbot.ar_cmd(pattern="معلومات_ألملصق$")
