...
@jakootelbot.ar_cmd(pattern="أيقاف السورس$")
async def _(event):
    if BOTLOG:
        await event.client.send_message(
...
@jakootelbot.ar_cmd(pattern="أيقاف مؤقت( [0-9]+)?$")
async def _(event):
...
@jakootelbot.ar_cmd(pattern="الاشعارات (تشغيل|ايقاف)$")
