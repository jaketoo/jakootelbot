...
@jakootelbot.ar_cmd(pattern="اوامر التحكم")
async def hi(event):
    await edit_or_reply(
        event,
...
@jakootelbot.ar_cmd(pattern="التحكم (تفعيل|تعطيل)$")
...
@jakootelbot.ar_cmd(pattern="اضف متحكم(?:\s|$)([\s\S]*)")
...
@jakootelbot.ar_cmd(pattern="ازالة متحكم(?:\s|$)([\s\S]*)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
@jakootelbot.ar_cmd(pattern="المتحكمين$")
async def _(event):
...
jakootelbot.loop.create_task(_init())
