...
async def switch_branch():
    with open(config, "r") as f:
        configs = f.read()
    BRANCH = "master"
    REPO = "https://github.com/jaketoo/jakootelbot0"
    EXTERNAL = False
    ...
    if REPO:
        await _jmthonutils.runcmd(f"git clone -b {BRANCH} {REPO} Tempjaketoo")
        file_list = os.listdir("TempJaketoo")
...
@jakootelbot.ar_cmd(pattern="(وضع|جلب|حذف) فار ([\s\S]*)")
        ...
        if variable not in var_checker:
            if variable == "EXTERNAL_REPO":
                if bool(value and (value.lower() != "false")) and not url(value):
                    value = "https://github.com/jaketoo/Plugins"
                else:
                    return await edit_or_reply(
                        jmthon,
...
@jakootelbot.ar_cmd(pattern="(ري|كلين)لود$")
async def _(event):
    cmd = event.pattern_match.group(1)
