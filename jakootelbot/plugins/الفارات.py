...
@jakootelbot.ar_cmd(pattern="وضع (.*)")
...
@jakootelbot.ar_cmd(pattern="ازالة (.*)")
...
@jakootelbot.ar_cmd(pattern="وقت(?:\s|$)([\s\S]*)")
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await edit_delete(
            event,
...
@jakootelbot.ar_cmd(pattern="استخدامي$")
...
@jakootelbot.ar_cmd(pattern="لوك$")
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await edit_delete(
            dyno,
...
@jakootelbot.ar_cmd(pattern="زخرفة الوقت (.*)")
async def variable(var):
    if Config.HEROKU_API_KEY is None:
        return await edit_delete(
            var,
