...
@jakootelbot.ar_cmd(pattern="اكينوتر(?:\s|$)([\s\S]*)")
async def rozdo(e):
...
@jakootelbot.tgbot.on(CallbackQuery(data=re.compile(b"aki_?(.*)")))
@check_owner
async def daj(e):
...
@jakootelbot.tgbot.on(CallbackQuery(data=re.compile(b"aka_?(.*)")))
@check_owner
async def rooks(e):
