...
from jakootelbot import Convert, jakootelbot
...
@jakootelbot.ar_cmd(pattern="دائري ?((-)?s)?$")
...
@jakootelbot.ar_cmd(pattern="(لصورة|تحويل صورة)$")
async def _(event):
...
@jakootelbot.ar_cmd(pattern="(لملصق|تحويل ملصق)$")
async def _(event):
    reply_to_id = await reply_id(event)
    reply = await event.get_reply_message()
    if not reply:
...
@jakootelbot.ar_cmd(pattern="لملف ([\s\S]*)")
async def get(event):
    name = event.text[5:]
    if name is None:
...
@jakootelbot.ar_cmd(pattern="لكتابة$")
async def get(event):
    reply = await event.get_reply_message()
    mediatype = await media_type(reply)
...
@jakootelbot.ar_cmd(pattern="الملف لصورة$")
...
@jakootelbot.ar_cmd(pattern="(تحويل متحركة|لمتحركة)$")
async def _(event):
...
@jakootelbot.ar_cmd(pattern="تحويل الى (صوتي|بصمة)")
async def _(event):
