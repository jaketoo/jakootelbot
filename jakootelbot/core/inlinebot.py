...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**jakootelbot"):
            buttons = [
                (
            ...
            builder = event.builder
            troll = os.path.join("./jakootelbot", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./jakootelbot", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./jakootelbot", "hide.txt")
            try:
...
@jakootelbot.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@jakootelbot.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@jakootelbot.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@jakootelbot.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
